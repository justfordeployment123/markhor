  ...
  return (
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-xl p-8 w-full max-w-md">
        ...
        <p className="text-caption text-gray-700 mt-4 text-center">
          Already have an account? <a href={inviteToken ? `/login?invite=${inviteToken}&email=${encodeURIComponent(inviteEmail || '')}` : '/login'} className="text-indigo-600 underline">Sign in</a>
        </p>
      </form>
    </div>
  );
