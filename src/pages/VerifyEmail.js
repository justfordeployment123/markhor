  ...
  return (
      ...
      <form onSubmit={handleVerify} className="bg-white rounded-3xl shadow-xl p-8 w-full max-w-md">
        ...
        <div className="mb-4">
          ...
          <input type="text" required value={token} onChange={e => setToken(e.target.value)} placeholder="Paste token" className="w-full px-4 py-3 rounded-xl border border-gray-400 focus:ring-2 focus:ring-indigo-500 text-gray-900 bg-gray-50" />
        </div>
        <div className="mb-4">
          <label className="block text-gray-800 font-semibold mb-2">Email (for resend)</label>
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com" className="w-full px-4 py-3 rounded-xl border border-gray-400 focus:ring-2 focus:ring-indigo-500 text-gray-900 bg-gray-50" />
        </div>
        ...
        <button type="submit" disabled={loading} className="w-full py-3 px-6 bg-gradient-to-r from-[#818cf8] to-[#a78bfa] text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300">
